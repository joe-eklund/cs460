class SendBuffer(object):
    ...
    def get(self,size):
        ''' Get the next data that has not been sent yet. Return the
            ...
            be less.'''
        # sets the packet size to smaller than param if data remaining is less
        if self.next + size > self.last:
            size = self.last - self.next
        # index of the data we're getting
        start = self.next - self.base
        # get the data
        data = self.buffer[start:start+size]
        # set sequence to the data we just retrieved
        sequence = self.next
        # move the pointer for the next chunk to be the next un-retrieved data
        self.next = self.next + size
        # return the data and the sequence number for the front of the data we're returning
